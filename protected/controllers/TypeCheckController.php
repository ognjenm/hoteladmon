...
class TypeCheckController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','index','view','delete','getTypeCheck'),
				'users'=>array('@'),
			),
    ...
    public function actionGetTypeCheck(){

        $res=array('ok'=>false,'concept'=>array(''=>Yii::t('mx','Select')));
        ...
        if(Yii::app()->request->isPostRequest){
            ...
            if($options) $res=array('ok'=>true,'concept'=>$options);
	...
	public function actionCreate()
	{
		...
		{
			$model->attributes=$_POST['TypeCheck'];
			if($model->save())
	...
	{
        $model=new TypeCheck('search');
        $model->unsetAttributes();  // clear any default values
        if(isset($_GET['TypeCheck']))
        $model->attributes=$_GET['TypeCheck'];
	...
	{
		$model=TypeCheck::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='type-check-form')
		{

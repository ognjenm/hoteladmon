...
class PollController extends Controller
{
	...
	{
		$model=new Poll;
        $description=Yii::t('mx','Lodging').' ';
        $res=array('ok'=>false,
            'redirect'=>Yii::app()->createUrl('reservation/index')
        );
        ...
        if(Yii::app()->request->isAjaxRequest){
            ...
            foreach($formats as $item) $description.=Yii::t('mx',$item['service_type']).', ';

            $charges=new Charges;
            $charges->customer_reservation_id=$CustomerReservationId;
            $charges->concept_id=1;

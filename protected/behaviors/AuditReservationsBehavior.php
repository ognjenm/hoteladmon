...
class AuditReservationsBehavior extends CActiveRecordBehavior{
    ...
    public function auditAttributes($newattributes, $oldattributes = array()){
        ...
        $customerReservationId=$this->getOwner()->getIsNewRecord() ? $newattributes['customer_reservation_id'] : $oldattributes['customer_reservation_id'];
        $add=$this->getOwner()->getIsNewRecord() ? $newattributes['description'] : $oldattributes['description'];
        ...
        foreach ($newattributes as $name => $value) {
            ...
            if ($value != $old) {
                $this->leaveTrail($this->getOwner()->getIsNewRecord() ? 'SET' : 'CHANGE', $name, $value, $old,$customerReservationId,$add);
            }
        }
    }
    ...
    public function leaveTrail($action, $name = null, $value = null, $old_value = null,$customerReservationId=null,$add=0){
        if($action=='SET' and $add=='1'){

            $log                        = new AuditReservations();
            ...
            $log->customer_reservation_id=$customerReservationId;
            return $log->save();
